...
function add_floating_button_scripts()
{
    global $post;
    if ($post !== null) {
        // $postがnullでない場合、メタデータを取得
        $display = get_post_meta($post->ID, '_floating_button_display', true);
    } else {
        // $postがnullの場合、デフォルト値を使用するか、処理をスキップ
        $display = '1';
    }


    // デフォルトは表示（データベースからの設定が 'no' でなければ表示）
    if ($display !== 'no') {
        ...
        $containerBgColor = $options['container_bg_color'] ?? '#FFFFFF';  // コンテナの背景色を取得
        $closeButton = !empty($options['close_button']) ? '1' : '0';  // 閉じるボタンの表示を取得、チェックされていれば'1', そうでなければ'0'
        $numButtons = $options['columns'] ?? '1';  // 列の数をボタンの数として設定
        $design = $options['design'] ?? 'default';  // ボタンデザインを取得
        ...
        $presets = get_button_color_presets();  // プリセットカラーの取得



        $data = [
            'columns' => $numButtons,
            'numButtons' => $numButtons,  // 列の数に応じたボタンの数
            'containerBgColor' => $containerBgColor,  // コンテナの背景色
            'closeButton' => $closeButton,  // 閉じるボタンの表示
            'microcopy' => $options['microcopy'] ?? '',  // マイクロコピー
            'microcopyPosition' => $options['microcopy_position'] ?? 'left',  // マイクロコピーの位置
            'design' => $design,  // ボタンデザイン
        ...
        for ($i = 1; $i <= $numButtons; $i++) {
            $use_banner = $options["use_banner_$i"] ?? '0';  // バナー画像を使用するかどうかの設定を取得
            $image_id = $use_banner === '1' ? ($options["image_id_$i"] ?? '') : '';
            $image_url = $image_id ? wp_get_attachment_url($image_id) : '';  // 画像IDがあればURLを取得、なければ空
            ...
            $button_data = [
                ...
                'imageUrl' => $image_url  // 画像URLを追加
            ];
            $data['buttons'][] = $button_data;  // 各ボタンのデータを配列に追加
        }
...
function floating_button_admin_scripts($hook_suffix)
{
    ...
    if ('toplevel_page_floating-button-settings' === $hook_suffix) {
        ...
        $options = get_option('floating_button_settings', []);
        $numButtons = $options['columns'] ?? 1; // デフォルト値は1とする
        $license_status = get_option('wp_floating_button_license_status', 'invalid');
                        ...
                        <div class="floating-button-settings-group floating-button-field-<?php echo $i; ?>">
                            ...
                            <div class="image-upload-settings" style="<?php echo empty($use_banner) ? 'display: none;' : ''; ?>">
                                ...
                                <!-- <p class="recommended-size">
                                    【推奨バナーサイズ】<br>
                                    ボタン1つの場合: 幅500px 高さ100px;<br>
                                    ボタン2つの場合: 幅250px 高さ100px;<br>
                                    ボタン3つの場合: 幅166px 高さ100px;
                                </p> -->
                            </div>
                                ...
                                <div class="preset-color">
                                    ...
                                    <select id="floating_button_preset_<?php echo $i; ?>" name="floating_button_settings[preset_<?php echo $i; ?>]" class="preset-color-selector">
                                        ...
                                        foreach ($presets as $key => $colors) {
                                            // カスタムカラーかつライセンスが無効の場合はdisabled属性を追加
